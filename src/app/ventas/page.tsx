  ...
  const filteredVentas = useMemo(() => {
    ...
    return ventas.filter(
      (v) =>
        ...
        v.tienda.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [ventas, searchTerm]);
    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Análisis de sell-out por tienda y producto
        </p>
      </div>
      ...
      {/* Tabs for Top Products / Top Stores */}
                  ...
                  {top_productos.map((producto, index) => (
                    <TableRow key={producto.upc}>
                      ...
                      <TableCell className="font-medium">
                        {formatProductName(producto.producto)}
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <Badge variant="secondary">
                          {((producto.monto / kpis.ventas_totales) * 100).toFixed(1)}%
                        </Badge>
                      </TableCell>
                    </TableRow>
                  ))}
                ...
                <TableBody>
                  {top_tiendas.map((tienda, index) => (
                    <TableRow key={tienda.id_tienda}>
                      ...
                      <TableCell className="text-right tabular-nums">
                        ${tienda.monto.toLocaleString("es-MX")}
                      </TableCell>
                      <TableCell className="text-right tabular-nums">
                      ...
                      <TableCell className="text-right">
                        <Badge variant="secondary">
                          {((tienda.monto / kpis.ventas_totales) * 100).toFixed(1)}%
                        </Badge>
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <CardTitle className="text-base font-medium">
              Detalle de Ventas por Tienda-Producto
            </CardTitle>
            <input
              type="text"
              placeholder="Buscar..."
              className="flex h-9 w-full sm:w-64 rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              value={searchTerm}
                ...
                {filteredVentas.slice(0, 50).map((venta, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className="text-sm">
                      {formatProductName(venta.producto)}
                    </TableCell>
                    <TableCell className="text-right tabular-nums">
                      ${venta.monto.toLocaleString("es-MX")}
                    </TableCell>
                    <TableCell className="text-right tabular-nums">
                      {venta.unidades.toLocaleString()}
                    </TableCell>
                    <TableCell className="text-right tabular-nums">
                      ${venta.precio_promedio.toFixed(2)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
      ...
      {filteredVentas.length > 50 && (
        <p className="text-center text-sm text-muted-foreground mt-4">
          Mostrando 50 de {filteredVentas.length} registros
        </p>
      )}
    </div>
  );
